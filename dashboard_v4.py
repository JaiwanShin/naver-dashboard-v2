...
# ===== V4 이상치 분석 옵션 =====
st.sidebar.markdown("---")
st.sidebar.subheader("🔬 V4 이상치 분석 옵션")

include_variants = st.sidebar.checkbox(
    "변형 용량 포함", 
    value=False,
    help="체크하면 대표 용량 외 변형 용량(NON_STANDARD_SIZE)도 포함합니다"
)

use_aux = st.sidebar.checkbox(
    "보조 이상치 규칙 사용", 
    value=False,
    help="체크하면 median 대비 ±X% 이상 편차도 이상치로 표시합니다"
)

aux_pct = st.sidebar.number_input(
    "보조 규칙 임계값 (%)",
    min_value=10,
    max_value=100,
    value=50,
    step=5,
    disabled=not use_aux,
    help="median 대비 ±X% 이상이면 이상치로 표시"
)

# 허용범위 방식 선택 (IQR vs Quantile Cap)
...
def reorder_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 앞쪽 컬럼
    for col in priority_front:
        if col in existing:
            ordered.append(col)
            existing.remove(col)
    
    # 중간 컬럼
    for col in priority_middle:
        if col in existing:
            ordered.append(col)
            existing.remove(col)
...
@st.cache_data
def process_product_data(_df_raw, query: str, include_variants: bool, use_aux: bool, aux_pct: float, outlier_method: str = "iqr"):
    """V4 파이프라인: 컬럼매핑 → 판매처식별 → 필터링 → 이상치탐지
    
    Args:
        outlier_method: "iqr" (IQR 기존) 또는 "quantile" (Q2.5 Quantile Cap)
    """
    # 1) 컬럼 매핑
    df = map_columns(_df_raw)
    # 2) 판매처 식별
    df = add_seller_fields(df)
    # 3) 필터링
    df_kept, df_excluded, size_mode = filter_search_results(df, query, include_variants)
    
    # 4) 이상치 탐지 (방식 선택)
    if outlier_method == "quantile":
        df_before_outlier, df_inliers, df_outliers, stats_df = detect_outliers_quantile(
            df_kept, group_cols=['query'], use_aux=use_aux, aux_pct=aux_pct
        )
        method_label = "Quantile Cap (Q3)"
    else:
...
# ===== 탭 3: 상품 검색 (V4) =====
with tab3:
    st.subheader("📦 상품 검색 및 가격 분석 (V4)")
    st.markdown("네이버 쇼핑에서 상품을 검색하고 이상치 분석을 수행합니다.")
    ...
    if st.button("📦 상품 분석 (V4)", type="primary", key="product_analyze"):
        with st.spinner(f"'{product_query}' 상품 분석 중... (최대 {max_products}개)"):
            try:
                df_raw = client.search_all_products(query=product_query, max_results=max_products, sort=sort_option)
                if not df_raw.empty:
                    # V4 파이프라인 실행
                    results = process_product_data(
                        df_raw, 
                        query=product_query,
                        include_variants=include_variants,
                        use_aux=use_aux,
                        aux_pct=float(aux_pct),
                        outlier_method=outlier_method
                    )
    ...
    if st.session_state.analysis_results.get("tab3"):
        ...
        df_inliers = res.get("df_inliers", pd.DataFrame())
        df_outliers = res.get("df_outliers", pd.DataFrame())
        df_excluded = res.get("df_excluded", pd.DataFrame())
        stats_df = res.get("stats_df", pd.DataFrame())
        seller_summary = res.get("seller_summary", pd.DataFrame())
        size_mode = res.get("size_mode")
        df_before_outlier = res.get("df_before_outlier", pd.DataFrame())
        
        # 분석 완료 메시지
        total_count = len(df_inliers) + len(df_outliers)
        st.success(f"✅ V4 분석 완료! 정상: {len(df_inliers)}개 / 이상치: {len(df_outliers)}개 / 제외: {len(df_excluded)}개")
        
        # ===== Expander 1: 이상치 제외 데이터 (정상) =====
        method_label = res.get("method_label", "IQR")
        with st.expander(f"✅ 이상치 제외 데이터 (정상) - {method_label} 방식", expanded=True):
            # 상단 stats 요약
            if not stats_df.empty:
                q1_val = stats_df['Q1'].iloc[0] if 'Q1' in stats_df.columns else 0
                lower_val = stats_df['lower'].iloc[0] if 'lower' in stats_df.columns else 0
                upper_val = stats_df['upper'].iloc[0] if 'upper' in stats_df.columns else 0
                median_val = stats_df['median'].iloc[0] if 'median' in stats_df.columns else 0
                
                # IQR 방식인 경우 추가 정보
                if 'IQR' in stats_df.columns:
                    q3_val = stats_df['Q3'].iloc[0] if 'Q3' in stats_df.columns else 0
                    iqr_val = stats_df['IQR'].iloc[0] if 'IQR' in stats_df.columns else 0
                    method_info = f"Q1={q1_val:,.0f}, Q3={q3_val:,.0f}, IQR={iqr_val:,.0f}"
                else:
                    # Quantile 방식인 경우
                    upper_q_val = stats_df['upper_q'].iloc[0] if 'upper_q' in stats_df.columns else upper_val
                    method_info = f"Q1={q1_val:,.0f}, Q3(상한)={upper_q_val:,.0f}"
                
                st.info(f"""
                📊 **이상치 분석 요약** (방식: **{method_label}**)
                - 제거된 이상치: **{len(df_outliers)}개**
                - 정상 데이터: **{len(df_inliers)}개**
                - 대표 용량: **{size_mode}매** {"(변형 포함)" if include_variants else ""}
                - {method_label} 기준: {method_info}
                - **허용 범위: {lower_val:,.0f} ~ {upper_val:,.0f}원**
                """)
                
                # ===== 검증: 실제 max price가 upper 이하인지 확인 =====
                if not df_inliers.empty and 'price' in df_inliers.columns:
                    actual_max = df_inliers['price'].max()
                    if actual_max > upper_val + 1:  # 소수점 오차 허용
                        st.error(f"⚠️ **검증 실패**: 실제 최대가 **{actual_max:,.0f}원**이 허용범위 상한 **{upper_val:,.0f}원**을 초과합니다!")
                    else:
        ...
        # ===== 가격 통계 (정상 데이터 기준) =====
        st.subheader("💰 가격 통계 (정상 데이터 기준)")
        if not df_inliers.empty and 'price' in df_inliers.columns:
            col1, col2, col3, col4 = st.columns(4)
            col1.metric("최저가", f"{df_inliers['price'].min():,.0f}원")
            col2.metric("최고가", f"{df_inliers['price'].max():,.0f}원")
            col3.metric("평균가", f"{df_inliers['price'].mean():,.0f}원")
            col4.metric("중앙값", f"{df_inliers['price'].median():,.0f}원")
        else:
            st.warning("가격 통계를 계산할 수 없습니다.")
        
        # ===== 가격 분포 차트 (정상 데이터 기준) =====
        if not df_inliers.empty and 'price' in df_inliers.columns:
            st.subheader("📊 가격 분포 (정상 데이터)")
            fig = px.histogram(
                df_inliers, x="price", nbins=30,
                labels={"price": "가격 (원)", "count": "상품 수"},
                template="plotly_dark", color_discrete_sequence=["#3b82f6"]
            )
            fig.add_vline(x=df_inliers["price"].median(), line_dash="dash", line_color="#4ade80", 
                          annotation_text=f"중앙값: {df_inliers['price'].median():,.0f}원")
            fig.update_layout(
                paper_bgcolor='rgba(0,0,0,0)',
                plot_bgcolor='rgba(0,0,0,0)',
                font_family="Pretendard",
            ...
            st.info(f"""
            💡 **V4 데이터 가이드**:
            - **정상 데이터 기준**: IQR 방식으로 이상치를 제거한 정상 데이터만 표시합니다.
            - **중앙값**: {df_inliers['price'].median():,.0f}원을 기준으로 상품들이 분포해 있습니다.
            - **대표 용량**: {size_mode}매 기준으로 필터링되었습니다.
            """)
        
        # ===== 참고: 원본 데이터 통계 =====
        with st.expander("📋 원본 데이터 통계 (필터 후, 이상치 포함)"):
            st.caption("⚠️ 이상치 제거 전 데이터입니다. 참고용으로만 사용하세요.")
            if not df_before_outlier.empty and 'price' in df_before_outlier.columns:
                col1, col2, col3, col4 = st.columns(4)
                col1.metric("원본 최저가", f"{df_before_outlier['price'].min():,.0f}원")
                col2.metric("원본 최고가", f"{df_before_outlier['price'].max():,.0f}원")
                col3.metric("원본 평균가", f"{df_before_outlier['price'].mean():,.0f}원")
                col4.metric("원본 중앙값", f"{df_before_outlier['price'].median():,.0f}원")
        
        # ===== 브랜드/판매처 분석 (정상 데이터 기준) =====
        if not df_inliers.empty:
            col1, col2 = st.columns(2)
            with col1:
                if 'brand' in df_inliers.columns:
            ...
            with col2:
                if 'mall_name' in df_inliers.columns:
                    mall_counts = df_inliers["mall_name"].value_counts().head(10)
                    if not mall_counts.empty:
                        fig = px.bar(x=mall_counts.values, y=mall_counts.index, orientation="h", title="판매처 분포 (Top 10, 정상 데이터)", template="plotly_dark", color=mall_counts.values, color_continuous_scale="Blues")
        ...
        export_dfs = {}
        if not df_inliers.empty:
            export_dfs["정상데이터"] = reorder_columns(df_inliers)
        if not df_outliers.empty:
            export_dfs["이상치"] = reorder_columns(df_outliers)
        if not df_excluded.empty:
            export_dfs["제외데이터"] = reorder_columns(df_excluded)
        if not seller_summary.empty:
            export_dfs["판매처요약"] = seller_summary
        
        if export_dfs:
            create_excel_download(export_dfs, f"상품분석_V4_{p_query}", key="tab3_download")
